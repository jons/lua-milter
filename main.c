...
static int Milter_smfi_getsymval (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_getsymval: invalid envelope");
    lua_error(S);
  }
...
static int Milter_smfi_setreply (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_setreply: invalid envelope");
    lua_error(S);
  }
...
{
  SMFICTX *ctx;
  long r = MI_FAILURE, i, nargs, n;
  ffi_cif ci;
  ffi_type **t_args;
  void **args, **f_args;
  n = lua_gettop(S);
  nargs = n+1;// setmlreply expects an additional NULL at the end
  if (n < 4)
  {
    lua_pushliteral(S, "smfi_setmlreply: missing argument");
    lua_error(S);
  }
  ctx = unwrap_envelope(S, n+1);
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_setmlreply: invalid envelope");
    lua_error(S);
  }
  t_args = malloc(nargs * sizeof(ffi_type *));
  f_args = malloc(nargs * sizeof(void *));
  ...
  args[0] = ctx;
  args[1] = (char *)lua_tostring(S, 2);
  args[2] = (char *)lua_tostring(S, 3);
  for (i = 3; i < n; i++)
    args[i] = (char *)lua_tostring(S, i+1);
  args[n] = NULL;

  if (FFI_OK == ffi_prep_cif(&ci, FFI_DEFAULT_ABI, nargs, &ffi_type_sint, t_args))
    ffi_call(&ci, FFI_FN(smfi_setmlreply), &r, f_args);
  else
    fprintf(stderr, "smfi_setmlreply: ffi_prep_cif failed\n");
...
static int Milter_smfi_addheader (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_addheader: invalid envelope");
    lua_error(S);
  }
...
static int Milter_smfi_chgheader (lua_State *S)
{
  ...
  if (n < 4)
  {
    lua_pushliteral(S, "smfi_chgheader: missing argument");
    lua_error(S);
  }
  ctx = unwrap_envelope(S, n+1);
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_chgheader: invalid envelope");
    lua_error(S);
  }
  headerf = (char *)lua_tostring(S, 2);
...
static int Milter_smfi_insheader (lua_State *S)
{
  ...
  if (n < 4)
  {
    lua_pushliteral(S, "smfi_insheader: missing argument");
    lua_error(S);
  }
  ctx = unwrap_envelope(S, n+1);
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_insheader: invalid envelope");
    lua_error(S);
  }
  hdridx = lua_tointeger(S, 2);
  headerf = (char *)lua_tostring(S, 3);
  headerv = (char *)lua_tostring(S, 4);
  r = smfi_insheader(ctx, hdridx, headerf, headerv);
  lua_pushinteger(S, r);
  return 1;
}
...
static int Milter_smfi_chgfrom (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_chgfrom: invalid envelope");
    lua_error(S);
  }
...
static int Milter_smfi_addrcpt (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_addrcpt: invalid envelope");
    lua_error(S);
  }
...
static int Milter_smfi_addrcpt_par (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_addrcpt_par: invalid envelope");
    lua_error(S);
  }
  rcpt = (char *)lua_tostring(S, 2);
  args = (char *)lua_tostring(S, 3);
...
static int Milter_smfi_delrcpt (lua_State *S)
{
  ...
  if (n < 2)
  {
    lua_pushliteral(S, "smfi_delrcpt: missing argument");
    lua_error(S);
  }
  ctx = unwrap_envelope(S, n+1);
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_delrcpt: invalid envelope");
    lua_error(S);
  }
  rcpt = (char *)lua_tostring(S, 2);
...
static int Milter_smfi_replacebody (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_replacebody: invalid envelope");
    lua_error(S);
  }
...
static int Milter_smfi_progress (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_progress: invalid envelope");
    lua_error(S);
  }
  r = smfi_progress(ctx);
  lua_pushinteger(S, r);
...
static int Milter_smfi_quarantine (lua_State *S)
{
  ...
  if (NULL == ctx)
  {
    lua_pushliteral(S, "smfi_quarantine: invalid envelope");
    lua_error(S);
  }
